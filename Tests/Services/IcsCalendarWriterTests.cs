...
/// <summary>
/// ICS 写入器。整份从 XAUAT.LoginApi 迁来（逻辑与用例名都逐字保留），
/// 它守的是"迁移前后字节一致"这条硬要求。
/// </summary>
public class IcsCalendarWriterTests
{
    ...
    [Fact]
    public void Create_ShouldEmitEduApiProdid()
    {
        // 迁移后生成方变成了 EduApi。PRODID 不参与事件身份（去重看 UID），
        // 所以老订阅者不会因此看到重复事件；LoginApi 原来的用例没钉这一条，这里补上。
        var ics = Create();

        Assert.Contains("PRODID:-//XAUAT//EduApi//CN", ics, StringComparison.Ordinal);
        Assert.DoesNotContain("LoginApi", ics, StringComparison.Ordinal);
    }
