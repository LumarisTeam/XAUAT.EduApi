...
/// 提醒弹窗的文案。**与 <see cref="Summary"/> 不同**——课程和考试用了两套措辞
...
/// ICS（iCalendar）序列化。
/// <para>
/// 从 XAUAT.LoginApi 原样迁移而来（逻辑逐行不变）。<b>为什么手写而不引库</b>：
/// 这份输出已经是线上几万订阅者的既成事实，<c>UID</c> 必须与迁移前逐字一致，
/// 引入 Ical.Net 会改变字节输出并有产生重复事件的风险。RFC 5545 的这部分又足够简单，
/// 可控性反而更好。
/// </para>
/// <para>
/// 与 Flask（<c>icalendar</c> 库）相比，本实现做了**规范化**：
/// </para>
/// <list type="bullet">
...
/// </list>
/// <para>
/// <c>UID</c> 格式与 Flask/LoginApi 保持逐字一致（<c>exam-{课程}-{起始时间}</c> /
/// <c>course-{lessonId}-{起始时间}</c>），否则已订阅的用户会在日历里看到重复事件。
/// </para>
/// </summary>
public static class IcsCalendarWriter
{
        ...
        IReadOnlyList<IcsEvent> events, string calendarName, string appleColor, DateTimeOffset generatedAt)
    {
        ...
        AppendLine(builder, "PRODID:-//XAUAT//EduApi//CN");
    ...
    /// <summary>把时间格式化成 ICS 的本地时间串（测试与调试用）。</summary>
